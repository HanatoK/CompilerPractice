...
ExpressionExecutor::ExpressionExecutor(Executor &executor): SubExecutorBase(executor), mValueType(VariableType::UNKNOWN)
{

}

std::shared_ptr<SubExecutorBase> ExpressionExecutor::execute(std::shared_ptr<ICodeNode<ICodeNodeTypeImpl, ICodeKeyTypeImpl> > node)
{
  const auto node_type = node->type();
  switch (node_type) {
    case ICodeNodeTypeImpl::VARIABLE: {
      const auto entry = std::any_cast<std::shared_ptr<SymbolTableEntry<SymbolTableKeyTypeImpl>>>(node->getAttribute(ICodeKeyTypeImpl::ID));
      mValue = entry->getAttribute(SymbolTableKeyTypeImpl::DATA_VALUE);
      mValueType = std::any_cast<VariableType>(entry->getAttribute(SymbolTableKeyTypeImpl::DATA_TYPE));
      break;
    }
...
std::any ExpressionExecutor::value() const
{
  return mValue;
}

VariableType ExpressionExecutor::valueType() const
{
  return mValueType;
}

std::shared_ptr<SubExecutorBase> ExpressionExecutor::executeBinaryOperator(std::shared_ptr<ICodeNode<ICodeNodeTypeImpl, ICodeKeyTypeImpl> > node, ICodeNodeTypeImpl node_type)
{
  ...
  ExpressionExecutor executor_lhs(*currentExecutor()), executor_rhs(*currentExecutor());
  ...
  if (search != mArithOps.end()) {
    ...
    if (integer_mode) {
      const unsigned long long lhs_value_int = std::any_cast<unsigned long long>(lhs_value);
      const unsigned long long rhs_value_int = std::any_cast<unsigned long long>(rhs_value);
      switch (node_type) {
        ...
        case ICodeNodeTypeImpl::FLOAT_DIVIDE: {
          if (rhs_value_int != 0) {
            mValue = static_cast<double>(lhs_value_int) / static_cast<double>(rhs_value_int);
            mValueType = VariableType::FLOAT;
          } else {
            mValue = 0.0;
            mValueType = VariableType::FLOAT;
            errorHandler()->flag(node, RuntimeErrorCode::DIVISION_BY_ZERO, currentExecutor());
          }
          break;
        }
      ...
      const double lhs_value_float =
          ...
          static_cast<double>(std::any_cast<unsigned long long>(lhs_value)):
          std::any_cast<double>(lhs_value);
      const double rhs_value_float =
          (type_rhs == VariableType::INTEGER) ?
          static_cast<double>(std::any_cast<unsigned long long>(rhs_value)):
      ...
      switch (node_type) {
        ...
        case ICodeNodeTypeImpl::FLOAT_DIVIDE: {
          ...
          if (rhs_value_float != 0.0) {
            mValue = static_cast<double>(lhs_value_float) / static_cast<double>(rhs_value_float);
          } else {
            mValue = 0.0;
            errorHandler()->flag(node, RuntimeErrorCode::DIVISION_BY_ZERO, currentExecutor());
          }
  ...
  } else if (node_type == ICodeNodeTypeImpl::AND || node_type == ICodeNodeTypeImpl::OR) {
    // and and or
